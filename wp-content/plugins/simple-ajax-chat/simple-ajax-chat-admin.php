...
function sac_export_chat_panel() {
	...
	if (sac_export_chats()) {
		...
		$output .= '<p><a target="_blank" href="'. esc_url($download) .'">'. esc_html__('Download CSV File', 'simple-ajax-chat') .'</a></p>';
...
function sac_render_form() {
	...
	<style type="text/css">
		...
		#mm-plugin-options .notice p { margin-left: 0; margin-right: 0; }
		...
		.mm-code { padding: 5px 10px; background-color: #fafae0; color: #333; font-size: 14px; }
		
		#mm-plugin-options .button-primary, #mm-plugin-options .button-secondary { margin: 0 0 15px 15px; }

		#mm-chat-latest { color: #339933; }
		#mm-plugin-options #mm-chat-list { margin: 20px 0 20px 15px; }
		#mm-chat-list li { width: 100%; overflow: hidden; padding: 1px 0; list-style-type: none; }
		#mm-chat-list input.regular-text { padding: 4px 5px; }
	...
	<div id="mm-plugin-options" class="wrap">
					...
					<div id="mm-panel-alert"<?php echo $display_alert; ?> class="postbox">
						...
						<div class="toggle">
							<div class="mm-panel-alert">
								<p>
									<?php esc_html_e('Please', 'simple-ajax-chat'); ?> <a target="_blank" href="https://m0n.co/donate" title="<?php esc_attr_e('Make a donation via PayPal!', 'simple-ajax-chat'); ?>"><?php esc_html_e('make a donation', 'simple-ajax-chat'); ?></a> <?php esc_html_e('and/or', 'simple-ajax-chat'); ?> 
									<a target="_blank" href="https://wordpress.org/support/plugin/simple-ajax-chat/reviews/?rate=5#new-post" title="<?php esc_attr_e('Thank you for your support!', 'simple-ajax-chat'); ?>"><?php esc_html_e('give it a 5-star rating', 'simple-ajax-chat'); ?>&nbsp;&raquo;</a>
								</p>
								<p>
						...
						<div class="toggle<?php if ($sac_action_any) echo ' default-hidden'; ?>">
							<div class="mm-panel-overview">
								...
								<ul>
									...
									<li><a target="_blank" href="https://wordpress.org/plugins/simple-ajax-chat/"><?php esc_html_e('Plugin Homepage', 'simple-ajax-chat'); ?></a></li>
								</ul>
								<p>
						...
						<div class="mm-table-wrap">
							...
							<?php if (!empty($chats)) :
								...
								foreach ($chats as $chat) : 
									...
									else $url = '<a target="_blank" href="'. $chat->url .'">'. $chat->name .'</a>';
									
									if ($i === 1) : ?>
									<div id="mm-chat-latest"><?php echo sprintf(esc_html__('Latest Message: %s ago', 'simple-ajax-chat'), sac_time_since($chat->time)); ?></div>
									<ul id="mm-chat-list">	
									<?php endif; ?>
		...
		<div id="mm-credit-info">
			<a target="_blank" href="<?php echo $sac_homeurl; ?>" title="<?php esc_attr_e('Plugin Homepage', 'simple-ajax-chat'); ?>"><?php echo $sac_plugin; ?></a> <?php esc_html_e('by', 'simple-ajax-chat'); ?> 
			<a target="_blank" href="https://twitter.com/perishable" title="<?php esc_attr_e('Jeff Starr on Twitter', 'simple-ajax-chat'); ?>">Jeff Starr</a> @ 
			<a target="_blank" href="https://monzillamedia.com/" title="<?php esc_attr_e('Obsessive Web Design &amp; Development', 'simple-ajax-chat'); ?>">Monzilla Media</a>
		</div>
	...
	<script type="text/javascript">
		jQuery(document).ready(function(){
			...
			jQuery('#mm-export-chats').click(function(){
				var r = confirm('<?php _e('Are you sure you want to create a new file on the server? It will contain sensitive data like user IP address. You can always delete the file by clicking on the "Delete CSV" link, which will appear after the file is created. Click "OK" to continue.', 'simple-ajax-chat'); ?>');
				if (r == true){
					return true;
				} else {
					return false;
